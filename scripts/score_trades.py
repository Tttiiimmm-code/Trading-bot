...
def summarise(rows: list[dict], risk_pct: float, label: str) -> None:
    if not rows:
    ...
    print(f"first / last    : {rows[0]['opened_at'][:16]} -> {rows[-1]['closed_at'][:16]}")
...
def main() -> None:
    ...
    if len(args.journals) > 1 and pooled:
        ...
        summarise(pooled, args.risk_pct, "all instances pooled")
        print("\nNote: each instance is its own simulated account, so the balance and net PnL")
        print("above are sums of separate accounts, not one portfolio's result.")
